...
dynamic_config = list(
  list(
    ...
    result = "warn",                 # Action on duplicate ("warn" or "stop")
    result_field = "title_warning"   # Field to show warning message (should be hidden)
  )
)
```
...
2. If a duplicate is found, either:
   ...
   - Prevent form submission (result = "stop")
      ...
      elements = list(
        ...
        list(
          ...
          html = "<div style='color: orange;'>
                   ⚠️ This issue may have been reported already. 
...
This configuration creates a robust form that:
- Tracks where users came from
- Warns about duplicate issue reports
- Provides package-specific version selection
...
This schema supports all the examples in this vignette while maintaining referential integrity and tracking creation dates.
